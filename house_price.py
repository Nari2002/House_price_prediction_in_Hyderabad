...
def predict_price(area,num_bedrooms,location):
    ...
    if label_encoded_location == -1:
        ...
        amount = price[0]
        return st.markdown(f'<h4 class="title1">Price Predicted Succesfully in {location}  </h4><br>', unsafe_allow_html=True),st.write(f'<h3 class="title">Price is ₹ {amount:,.2f} only/-</h3><br>', unsafe_allow_html=True)
